...
export default class LoadingBar {
  static start(userOptions = {}) {
    ...
    styles.textContent = `
      .${options.loadingBarClass}.off ~ * {
        opacity: 0;
        pointer-events: none;
        transition: none;
      }
      .${options.loadingBarClass} ~ * {
        opacity: 1;
      ...
      .${options.loadingBarClass} .innerbar {
        width: 0%;
        height: 100%;
        background: ${options.color};
        opacity: 1;
        transition: width ${options.incrementDelay +
          options.transitionDelay}ms ${options.transitionType};
        will-change: width;
      }
      .${options.loadingBarClass} .innerbar.shadow {
        box-shadow: ${options.shadowOptions} ${options.color};
      }
      .${options.loadingBarClass} .innerbar.smooth {
        transition: width ${options.endDelay}ms ease;
      }
      .number {
        position: fixed;
        top: 45px;
      ...
      .spinbar {
        ...
        height: 15px;
        border-radius: 50%;
        border: 4px solid ${options.spinBarColor};
    ...
    if (options.autoIncrement)
      this.timer = setInterval(
        () => this.go((this.progress += options.incrementValue)),
  ...
  static go(percent) {
    ...
    if (this.numberProgress)
      this.numberProgress.textContent = `${this.progress}%`;
